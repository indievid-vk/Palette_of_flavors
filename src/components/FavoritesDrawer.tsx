      ...
      <div className="glass-modal w-full max-w-lg h-full p-6 relative flex flex-col justify-between overflow-y-auto rounded-l-3xl rounded-r-none border-y-0 border-r-0">
        ...
        <div>
          ...
          <div className="flex items-center justify-between border-b border-white/60 pb-4 mb-6">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-full glass-button text-[#2C1E1C] hover:bg-white transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {favorites.length === 0 ? (
            <div className="py-16 text-center space-y-3">
              <Heart className="w-12 h-12 text-[#6B5A57]/30 mx-auto stroke-1" />
              <p className="text-sm font-medium text-[#2C1E1C]">
                У вас пока нет сохранённых вкусовых пар
              </p>
              <p className="text-xs text-[#6B5A57] max-w-xs mx-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs">
                <span className="text-[#6B5A57]">Ваша рабочая коллекция</span>
                <button
        ...
        <div className="pt-4 border-t border-[#4A2E2B]/10 mt-6 flex justify-end">
          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-xl bg-[#4A2E2B] text-white font-semibold text-xs hover:bg-[#3A2422] transition-colors"
          >
            Закрыть панель
          </button>
        </div>
